...
protocol TransactionsFilterCellDelegate: class {
    func transactionsFilterCell(_ : FilterTransactionsCell, didUpdateTimeFrame: DateInterval)
}

class FilterTransactionsCell: UITableViewCell {

    weak var delegate: TransactionsFilterCellDelegate?
    ...
    @objc func finishedPicking() {
        ...
        if didChangeTimeFrame {
            delegate?.transactionsFilterCell(self, didUpdateTimeFrame: timeFrames[rowPicked])
